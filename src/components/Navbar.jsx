...
const Navbar = () => {
    ...
    const menu = (
        <ul className="md:flex-row flex-col md:flex items-center space-x-4  md:space-y-0 space-y-4">
            <li>
    ...
    return (
        <nav
            className={`fixed top-0 left-0 w-full bg-green-400 z-50 transition-transform duration-300 ${visible ? 'transform-none' : '-translate-y-full'
                }`}
            aria-label="Main Navigation"
            role="navigation"
        >
            <div className="bg-gray-900 relative text-white shadow-md">
                ...
                <div
                    className={`absolute top-16 left-0 w-full bg-slate-400 transition-transform duration-300 ${menuOpen ? 'translate-x-0' : 'translate-x-full'
                        }`}
                >
                    {menu}
                </div>
